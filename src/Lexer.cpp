...
const std::unordered_map<std::string, TokenType> Lexer::keywords_ = {
    ...
    {"null", TokenType::NULL_TYPE},
    {"bunch", TokenType::BUNCH_TYPE},
    {"map", TokenType::MAP_TYPE},
    {"stack", TokenType::STACK_TYPE},
    {"queue", TokenType::QUEUE_TYPE},
    {"heap", TokenType::HEAP_TYPE},
    {"linkedlist", TokenType::LINKEDLIST_TYPE},
};

Lexer::Lexer(std::string source)
    : source_(std::move(source)), pos_(0), line_(1) {}
...
std::vector<Token> Lexer::tokenize() {
    ...
    while (current() != '\0') {
        ...
        if (ch == '*' && peek() == '*') {
            advance(); advance();
            tokens.emplace_back(TokenType::POWER, "**", line_);
            continue;
        }
        if (ch == '=' && peek() == '=') {
            advance(); advance();
            tokens.emplace_back(TokenType::EQEQ, "==", line_);
            continue;
        }
        if (ch == '!' && peek() == '=') {
            advance(); advance();
        ...
        switch (ch) {
            ...
            default:
                throw std::runtime_error(
                    "[Nova SyntaxError line " + std::to_string(line_) +
...
Token Lexer::readString() {
    ...
    while (true) {
        ...
        if (ch == '\0') {
            throw std::runtime_error(
                "[Nova SyntaxError line " + std::to_string(startLine) +
                "] Unterminated string"
            );
        }
        ...
        if (ch == '"') {
            ...
            //std::cout << "[LEXER] Read string: '" << str << "'\n";
        ...
        if (ch == '\\') {
            ...
            switch (next) {
                ...
                default:
                    throw std::runtime_error(
                        "[Nova SyntaxError line " + std::to_string(line_) +
                        "] Unknown escape: \\" + std::string(1, next)
                    );
            }
            advance();
        } else {
...
Token Lexer::readIdentOrKeyword() {
    ...
    if (it != keywords_.end()) {
        ...
        if (tt == TokenType::BOOL)
            return Token(TokenType::BOOL, word == "true", line_);
        if (tt == TokenType::NULLVAL)
            return Token(TokenType::NULLVAL, word, line_);
